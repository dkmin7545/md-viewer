...
```
src/
├── store/viewerStore.ts     # Zustand: files+kind, activeId, fontSize, theme, session 영속화
├── hooks/useFiles.ts        # 파일 업로드/드롭 (.md/.html/.jsx 자동 분류)
├── lib/peerSync.ts          # PeerJS 기반 sender/receiver (kind 포함)
├── components/
│   ├── TopBar.tsx           # 메뉴, 파일/폴더 열기, 📤 보내기, 📥 받기, 버전 표시
│   ├── ContentView.tsx      # kind에 따라 MarkdownView/HtmlView/JsxView로 디스패치
│   ├── MarkdownView.tsx     # .md 렌더 (max-width 680px)
│   ├── HtmlView.tsx         # .html을 sandbox iframe(allow-scripts only)에 렌더 + 원본 토글
│   ├── JsxView.tsx          # .jsx를 public/jsx-renderer iframe에 postMessage로 전달
│   ├── FilePanel.tsx        # 좌측 슬라이드 패널: 파일 목록 + kind 배지 + 전체삭제
│   ├── TocPanel.tsx         # 우측 슬라이드 패널: 헤딩 목차 (md용)
│   ├── FontSizeBar.tsx      # 하단 항상 보이는 폰트 슬라이더
│   ├── SendModal.tsx        # QR + 6자리 코드 표시
│   └── JoinPrompt.tsx       # 코드 입력 → 수신
└── App.tsx                  # 레이아웃 + 세션 hydrate + ?join=<code> URL 자동 처리

public/
└── jsx-renderer/            # 별도 jsx-renderer 프로젝트(Babel + lib loader) 임베드
    └── index.html           # ?embed=1 모드에서 부모 postMessage로 코드 수신
```
